...
export const getters: GetterTree<BTCPriceStore, RootState> = {
  ...
  btcPriceHistory: function (state, getters): { x: any; y: number }[] {
    return state.btcPrices.map((price) => {
      return {
        x: dayjs(price?.timestamp).format('h:mm:ss a'),
        y: price?.price / 100000000,
      }
    })
  },
}
...
export const actions: ActionTree<BTCPriceStore, RootState> = {
  ...
  async subscribeToBtcPrice(
    context,
    payload: { componentName: string; subscribe: boolean }
  ) {
    ...
    if (payload.subscribe) {
      ...
      if (context.state.subscribers.size == 1) {
        context.dispatch('getPriceInfo')
      }
    } else {
      ...
      if (context.state.subscribers.size == 0) {
        context.commit('CANCEL_POLLING_TIMER')
      }
    }
  },
